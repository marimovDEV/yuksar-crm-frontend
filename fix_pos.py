...
file_path = "src/components/sales/POS.tsx"
...
replacements = {
    r">MIJOZGA KO'RSATISH<": r">{t(\"Mijozga Ko'rsatish\").toUpperCase()}<",
    r">Yelim \(Glue\)<": r">{t(\"Yelim (Glue)\")}<",
    r">Dekorativ Panel<": r">{t(\"Dekorativ Panel\")}<",
}
...
if to_add:
    ...
    with open(trans_path, 'w') as f:
        f.write(new_content)
    print(f"Added {len(to_add)} translations for POS.")
else:
